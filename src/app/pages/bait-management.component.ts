...
@Component({
  ...
  template: `
    <div class="h-full w-full flex flex-col gap-8 p-4 animate-fade-in relative">
      <div>
        <h2 class="text-4xl font-serif font-black text-neo-ink tracking-tight">Catálogo de Iscas</h2>
        <p class="text-neo-ink font-bold text-sm mt-2 uppercase tracking-widest">Artificiais e Naturais</p>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 flex-1">
        <div class="neo-card p-8 flex flex-col h-fit">
          ...
          <form [formGroup]="baitForm" (ngSubmit)="salvar()" class="flex flex-col gap-5">
            <div>
              <label class="block text-sm font-black text-neo-ink uppercase tracking-wider mb-2">Nome da Isca *</label>
              <input type="text" formControlName="name" placeholder="Ex: Zara Superfície" class="neo-input">
            </div>
            <div>
            ...
            <div>
              ...
              <textarea formControlName="description" rows="3" placeholder="Características..." class="neo-input resize-none"></textarea>
            </div>
            <button type="submit" [disabled]="baitForm.invalid || salvando" class="neo-btn mt-4 w-full">
          ...
          <div class="neo-table-container">
            <table class="w-full text-left border-collapse neo-table">
              ...
              <tr *ngFor="let bait of baits">
                ...
                <td>
                  <span class="border-2 border-neo-ink font-black px-2 py-1 text-xs shadow-[2px_2px_0px_0px_#1D2B1F]"
                        [ngClass]="bait.type === 'ARTIFICIAL' ? 'bg-neo-lime' : 'bg-neo-paper'">
                      {{ bait.type }}
                    </span>
                </td>
                <td class="text-right">
                  <button (click)="abrirModalExclusao(bait.id)" class="neo-btn neo-btn-danger !px-4 !py-2 !text-xs">Excluir</button>
                </td>
              </tr>
              </tbody>
            </table>
            <div *ngIf="carregando" class="text-center py-8 font-bold text-neo-ink uppercase tracking-widest">Carregando iscas...</div>
          </div>
        </div>
      </div>
      ...
      <div *ngIf="idParaExcluir !== null" class="fixed inset-0 bg-neo-ink/80 flex items-center justify-center z-50 p-4">
        <div class="neo-card max-w-sm w-full p-8 flex flex-col items-center text-center animate-scale-up bg-white">
          ...
          <p class="text-neo-ink font-bold text-sm mb-8 leading-relaxed">Excluir esta isca apagará o registro do banco de dados.</p>
...
export class BaitManagementComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private baitService: BaitService, private cdr: ChangeDetectorRef) {
  ...
  ngOnInit(): void { this.carregarIscas(); }

  carregarIscas(): void {
    this.baitService.listarIscas(0, 50).subscribe({
      next: (p) => { this.baits = p.content; this.carregando = false; this.cdr.detectChanges(); },
  ...
  salvar(): void {
    ...
    this.baitService.salvarIsca(this.baitForm.value).subscribe({
      next: (b) => { this.baits.unshift(b); this.baitForm.reset({type: 'ARTIFICIAL'}); this.salvando = false; this.cdr.detectChanges(); },
