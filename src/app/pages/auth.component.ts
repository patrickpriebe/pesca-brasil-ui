    ...
    >
      ...
      <div class="neo-card w-full max-w-md p-8 relative z-30 animate-fade-in bg-white">
        <div class="flex flex-col items-center mb-8 border-b-[3px] border-neo-ink pb-6">
          <h1 class="text-4xl font-serif font-black text-neo-ink tracking-tighter">
            Pesca<span
              class="text-neo-lime"
              style="text-shadow: 1px 1px 0px #1D2B1F, -1px -1px 0px #1D2B1F, 1px -1px 0px #1D2B1F, -1px 1px 0px #1D2B1F;"
              >Brasil</span
            >
          </h1>
          <p class="text-neo-ink font-bold text-sm mt-2 tracking-widest uppercase">
            Compartilhe suas fisgadas
          </p>
        </div>

        <div
          *ngIf="mensagem"
          [ngClass]="sucesso ? 'bg-neo-lime' : 'bg-neo-rust text-white'"
          class="p-4 border-[3px] border-neo-ink text-sm font-bold text-center mb-6 animate-scale-up uppercase tracking-wide rounded-xl shadow-[4px_4px_0px_0px_#1D2B1F]"
        >
          {{ mensagem }}
        </div>

        <form
          *ngIf="telaAtual === 'LOGIN' || telaAtual === 'REGISTRO'"
          [formGroup]="authForm"
        ...
        >
          ...
          <div>
            ...
            <input
              ...
              placeholder="pescador@exemplo.com"
              class="neo-input"
            />
          </div>
          ...
          <div>
            <div class="flex justify-between items-center mb-2">
              <label class="block text-sm font-black text-neo-ink uppercase tracking-wider"
                >Senha</label
              >
              <button
                *ngIf="telaAtual === 'LOGIN'"
                type="button"
                (click)="mudarTela('ESQUECI_SENHA')"
                class="text-xs text-neo-rust font-black uppercase hover:underline transition-colors"
              >
                Esqueceu a senha?
              </button>
            </div>
            ...
            <div class="relative">
              <input
                ...
                placeholder="••••••••"
                class="neo-input pr-12"
              />
              <button
                type="button"
                (click)="toggleSenhaVisivel()"
                class="absolute inset-y-0 right-0 pr-4 flex items-center text-neo-ink focus:outline-none"
              >
                <svg
                  *ngIf="!senhaVisivel"
                  class="w-6 h-6"
          ...
          <div class="flex justify-center my-2">
            <re-captcha
              siteKey="6LfIKUAtAAAAAMF43nNxSPc5Q7YJV7jkH2TZrAYZ"
              (resolved)="onCaptchaResolvido($event)"
            ></re-captcha>
          </div>

          <button
            type="submit"
            [disabled]="authForm.invalid || carregando || !recaptchaToken"
            class="neo-btn w-full mt-2"
          >
            {{
              carregando
                ? 'Aguarde...'
        ...
        <div
          *ngIf="telaAtual === 'LOGIN' || telaAtual === 'REGISTRO'"
          class="mt-8 text-center text-sm font-bold text-neo-ink bg-neo-paper p-4 rounded-xl border-[3px] border-neo-ink"
        >
          {{ telaAtual === 'REGISTRO' ? 'Já tem uma conta?' : 'Ainda não é registrado?' }}<br />
          <button
            type="button"
            (click)="mudarTela(telaAtual === 'REGISTRO' ? 'LOGIN' : 'REGISTRO')"
            class="text-neo-rust font-black uppercase mt-1 hover:underline"
          >
...
export class AuthComponent implements OnInit {
  ...
  submitVerificacaoConta(): void {
    ...
    this.authService
      .verify({ email: this.emailDeTrabalho, code: this.otpForm.get('code')?.value })
      .subscribe({
        next: () => {
          this.sucesso = true;
  ...
  submitNovaSenha(): void {
    ...
    this.authService.resetPassword(payload).subscribe({
      ...
      error: (err) =>
        this.tratarErro(err, 'Erro ao redefinir a senha. Código inválido ou expirado.'),
    });
  }
}
