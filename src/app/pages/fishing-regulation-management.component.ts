          ...
          <form [formGroup]="regForm" (ngSubmit)="salvar()" class="flex flex-col gap-5">
            <div>
              <label class="block text-sm font-black text-neo-ink uppercase tracking-wider mb-2"
                >Bacia Hidrográfica *</label
              >
              <input
                type="text"
                formControlName="hydrographicBasin"
                placeholder="Ex: Bacia do Rio Uruguai"
                class="neo-input"
              />
            </div>
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  class="neo-input cursor-pointer uppercase text-xs"
                />
              </div>
              <div>
                <label class="block text-sm font-black text-neo-ink uppercase tracking-wider mb-2"
            ...
            <div>
              <label class="block text-sm font-black text-neo-ink uppercase tracking-wider mb-2"
                >Observações / Regras</label
              >
              <textarea
                formControlName="notes"
                rows="4"
                placeholder="Ex: Proibido abate..."
                class="neo-input resize-none"
              ></textarea>
            </div>
            <button
              type="submit"
              [disabled]="regForm.invalid || salvando"
              ...
              <tbody>
                <tr *ngFor="let reg of regulations">
                  <td>
                    ...
                    <div
                      class="text-xs font-bold text-neo-muted mt-1 uppercase"
                      [title]="reg.notes"
                    >
                      {{ reg.notes || 'SEM OBSERVAÇÕES' }}
                    </div>
                  </td>
                  ...
                  <td class="text-right">
                    <button
                      (click)="abrirModalExclusao(reg.id)"
      ...
      >
        ...
        >
          ...
          <p class="text-neo-ink font-bold text-sm mb-8 leading-relaxed">
            Você está prestes a excluir esta regra de pesca permanentemente.
          </p>
...
export class FishingRegulationManagementComponent implements OnInit {
  ...
  constructor(
    ...
    private regService: FishingRegulationService,
