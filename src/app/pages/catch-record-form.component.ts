...
@Component({
  ...
  template: `
    <div class="h-full w-full max-w-4xl mx-auto p-4 md:p-8 animate-fade-in relative">
      ...
      <div class="neo-card p-6 md:p-10 bg-white">
        <form [formGroup]="catchForm" (ngSubmit)="salvar()" class="flex flex-col gap-8">
            ...
            <div>
              ...
              <div class="flex gap-2">
                ...
                <button
                  ...
                  class="neo-btn !p-0 w-12 shrink-0 flex justify-center text-2xl"
                >
                  +
                </button>
              </div>
          ...
          <div
            class="p-4 border-[4px] border-neo-ink bg-neo-paper shadow-[8px_8px_0px_0px_#1D2B1F] relative"
          >
            <label class="block text-lg font-black text-neo-ink uppercase tracking-wider mb-1"
              >Ponto Exato da Captura *</label
            >
          ...
          <div
            class="border-[4px] border-dashed border-neo-ink hover:bg-neo-lime/10 p-8 text-center relative cursor-pointer min-h-[200px] flex flex-col items-center justify-center transition-colors bg-neo-paper shadow-inner mt-2"
          >
            <input
              type="file"
              accept="image/*"
            ...
            >
              ...
              <p class="text-xs font-bold text-neo-muted uppercase">
            ...
            >
              <img
                [src]="imagemPreview"
                class="max-h-64 object-cover border-[4px] border-neo-ink shadow-[6px_6px_0px_0px_#1D2B1F]"
              />
              <p
                class="text-xs text-neo-ink font-black mt-4 uppercase bg-white border-[3px] border-neo-ink px-4 py-2 shadow-[2px_2px_0px_0px_#1D2B1F]"
              >
                Trocar Foto
              </p>
            </div>
    ...
    <div
      *ngIf="modalAberto"
      class="fixed inset-0 z-[500] bg-neo-ink/80 flex items-center justify-center p-4"
    >
      <div class="neo-card w-full max-w-sm p-8 bg-white text-center">
        <h3 class="text-2xl font-serif font-black text-neo-ink mb-6 uppercase tracking-tight">
          Adicionar
          {{
            tipoModalCadastro === 'FISH'
              ? 'Espécie'
              : tipoModalCadastro === 'RIVER'
                ? 'Rio'
                : tipoModalCadastro === 'BAIT'
                  ? 'Isca'
                  : 'Equipamento'
          }}
        </h3>
        ...
        <input
          *ngIf="tipoModalCadastro !== 'EQUIPMENT'"
          type="text"
          [(ngModel)]="novoItemNome"
          placeholder="Nome..."
          class="neo-input mb-6 text-center"
        />
        ...
        <div *ngIf="tipoModalCadastro === 'EQUIPMENT'" class="flex flex-col gap-4 mb-6">
          ...
          <input
            type="text"
            [(ngModel)]="novoItemNome"
            placeholder="Ação / Marca..."
            class="neo-input text-center"
          />
        </div>
        ...
        <div class="flex gap-4">
          ...
          ><button
            ...
            [disabled]="
              !novoItemNome.trim() ||
              (tipoModalCadastro === 'EQUIPMENT' && !novoEquipamentoTipo) ||
              carregandoModal
            "
          >
...
export class CatchRecordFormComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  carregarDadosIniciais(): void {
    ...
    this.baitService.listarIscas(0, 100).subscribe((i) => {
      this.iscas = i.content;
      this.cdr.detectChanges();
    });
    this.equipmentService.listarEquipamentos(0, 100).subscribe((e) => {
